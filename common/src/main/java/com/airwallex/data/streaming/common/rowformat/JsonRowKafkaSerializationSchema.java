...
public class JsonRowKafkaSerializationSchema implements KafkaSerializationSchema<Row> {
    /**
     ...
     * @param typeInfo
     */
    public JsonRowKafkaSerializationSchema(RowTypeInfo typeInfo, String topic, int keyIndex) {
    ...
    private void initConnectNode() {
        ...
        try {
            ...
            String schemaTemplate = "{\"type\":\"struct\",\"fields\":[],\"optional\":false,\"name\":\"json-schema\"}";
    ...
    public static void main(String[] args) {
        new JsonRowKafkaSerializationSchema(null, " String topic", 1).initConnectNode();
    }


    @Override
    public ProducerRecord<byte[], byte[]> serialize(
            Row element, @Nullable Long timestamp
    ) {
        if (connectNode == null) {
            initConnectNode();
        }
        try {
            ...
            for (String field : typeInfo.getFieldNames()) {
                Object v = element.getField(typeInfo.getFieldIndex(field));
            ...
            return new ProducerRecord<byte[], byte[]>(topic, (element.getField(keyIndex)).toString().getBytes(), value);
        } catch (Throwable t) {
